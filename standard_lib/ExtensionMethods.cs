...
namespace standard_lib
{
    public static class ExtensionMethods
    {
        ...
        {
            if (await Task.WhenAny(Task.Delay(timeout), task).ConfigureAwait(false) == task)
            {
                return task.Result;
            }
            return defaultValue;
        ...
        /// <returns></returns>
        public static T FirstOrCustomException<T>(this IEnumerable<T> self, Func<T, bool> predicate, Exception exception)
        {
            try
            {
